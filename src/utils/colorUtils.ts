...
const analyzeImageStyle = (palette: number[][], colorAreas: Map<string, ColorWithArea>): ImageStyle => {
  ...
  const isMonochrome = (lowSatRatio > 0.8 && maxHighSatArea < 0.05) || 
                       (avgSaturation < 0.25 && maxHighSatArea < 0.05);
...
const selectDistinctColors = (
  candidates: Array<{ rgb: RGB; h: number; s: number; l: number; hex: string; score: number; areaRatio: number }>,
  count: number
): string[] => {
  if (candidates.length === 0) return [];
  
  const selected: Array<{ rgb: RGB; h: number; s: number; l: number; hex: string; score: number; areaRatio: number }> = [];
...
// 主要颜色提取函数
export const extractColors = async (imageElement: HTMLImageElement, count: number = 2): Promise<ColorExtractionResult> => {
  ...
  const candidates = palette
    .map(rgb => {
      ...
      return { rgb: rgb as RGB, h, s, l, hex: rgbToHex(r, g, b), score, areaRatio };
    })
  ...
  if (candidates.length > 0) {
    ...
    candidates.slice(0, 5).forEach((c, i) => {
      console.log(`  ${i + 1}. ${c.hex} | H:${(c.h * 360).toFixed(0)}° S:${(c.s * 100).toFixed(1)}% L:${(c.l * 100).toFixed(1)}% | 面积:${(c.areaRatio * 100).toFixed(2)}% | 评分:${c.score.toFixed(3)}`);
    });
  }
  ...
  distinctColors.forEach((hex, i) => {
    ...
    if (candidate) {
      console.log(`  选择 ${i + 1}: ${hex} | H:${(candidate.h * 360).toFixed(0)}° S:${(candidate.s * 100).toFixed(1)}% L:${(candidate.l * 100).toFixed(1)}%`);
    }
  });
  ...
  if (distinctColors.length < count) {
    ...
    const allCandidates = palette
      .map(rgb => {
        ...
        const key = `${r},${g},${b}`;
        const areaRatio = colorAreas.get(key)?.areaRatio || 0;
        return { rgb: rgb as RGB, h, s, l, hex: rgbToHex(r, g, b), score: areaRatio, areaRatio };
      })
